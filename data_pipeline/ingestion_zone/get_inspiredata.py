...
def download_and_process_municipality(url, epsg, output_gpkg, to_epsg=None):
    ...
    try:
        ...
        with zipfile.ZipFile(os.path.join(filename), "r") as z:
            z.extractall(path=os.path.join(os.curdir, 'data_pipeline/landing_zone/raw_data'))
        for gml in os.listdir('data_pipeline/landing_zone/raw_data'):
            if os.path.splitext(gml)[1] == '.gml':
                ...
                ogr_cmd = """ogr2ogr -update -append -f GPKG -s_srs EPSG:{} -t_srs EPSG:{} -lco IDENTIFIER={} {} {}""" \
                    .format(epsg, to_epsg, layer_name, output_gpkg + '.gpkg', os.path.join('data_pipeline/landing_zone/raw_data', gml))
                # print ("\n Executing: ", ogr_cmd)
...
def download(data_to_download, provincia=None, municipio=None, srs=None, filename="data_pipeline/landing_zone/buildings", separar_salida=False):
